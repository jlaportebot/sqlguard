...
from sqlguard.diff import Change, ChangeType, SchemaDiffer
...
class TestSchemaDiffer:
    ...
    def test_column_type_change_breaking(self):
        old = Schema(
            "v1",
            [Table("t", [Column("id", "integer", primary_key=True), Column("val", "integer")])],
        )
        new = Schema(
            "v2",
            [Table("t", [Column("id", "integer", primary_key=True), Column("val", "varchar")])],
        )
        differ = SchemaDiffer()
        diff = differ.diff(old, new)
    ...
    def test_column_type_widening_safe(self):
        old = Schema(
            "v1",
            [Table("t", [Column("id", "integer", primary_key=True), Column("val", "smallint")])],
        )
        new = Schema(
            "v2",
            [Table("t", [Column("id", "integer", primary_key=True), Column("val", "integer")])],
    ...
    def test_nullable_to_not_null_breaking(self):
        ...
        null_changes = [
            c for c in diff.changes if c.change_type == ChangeType.COLUMN_NULLABILITY_CHANGED
        ]
    ...
    def test_default_removed_breaking(self):
        ...
        new = Schema(
            ...
            [
                Table(
                    ...
                    [
                        Column("id", "integer", primary_key=True),
                        Column("status", "varchar", default=None),
                    ],
                )
            ],
        )
        ...
        default_changes = [
            c for c in diff.changes if c.change_type == ChangeType.COLUMN_DEFAULT_CHANGED
        ]
    ...
    def test_default_added_safe(self):
        old = Schema(
            "v1",
            [Table("t", [Column("id", "integer", primary_key=True), Column("status", "varchar")])],
        )
        new = Schema(
            "v2",
            [
    ...
    def test_unique_constraint_changes(self):
        ...
        uc_removed = [
            c for c in diff.changes if c.change_type == ChangeType.UNIQUE_CONSTRAINT_REMOVED
        ]
